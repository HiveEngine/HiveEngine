...
{
    //TODO: use a ressource manager with the ShaderHandle instead of copy the shader to the graphics pipeline

    Shader shader = {"shaders/vert.spv", "shaders/frag.spv"};

    create_shader(device_.logical_device, render_pass_.vk_render_pass, shader);
    if(shader.pipeline == VK_NULL_HANDLE) return false;


    graphics_pipeline_ = shader.pipeline;
