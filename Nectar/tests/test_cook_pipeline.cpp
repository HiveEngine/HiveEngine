...
namespace
{
                                   ...
                                   const char* path, const char* type, const void* data, size_t size)
    {
        ...
        record.m_path.Append(path, std::strlen(path));
        record.m_type = wax::String{alloc};
        record.m_type.Append(type, std::strlen(type));
    ...
    auto t1 = larvae::RegisterTest("NectarCookPipe", "CookSingleSuccess", []() {
        ...
        larvae::AssertEqual(result.m_cookedData.Size(), std::strlen(data));
    });
    ...
    auto t4 = larvae::RegisterTest("NectarCookPipe", "CookSingleNoIntermediate", []() {
        ...
        nectar::AssetRecord record{};
        record.m_uuid = id;
        record.m_path = wax::String{alloc, "test.dat"};
        record.m_type = wax::String{alloc, "TestType"};
        record.m_name = wax::String{alloc};
        record.m_labels = wax::Vector<wax::String>{alloc};
        db.Insert(static_cast<nectar::AssetRecord&&>(record));
    ...
    auto t6 = larvae::RegisterTest("NectarCookPipe", "CookAllSequential", []() {
        ...
        req.m_assets = wax::Vector<nectar::AssetId>{alloc};
        req.m_assets.PushBack(a);
        req.m_assets.PushBack(b);
        req.m_platform = "pc";
        req.m_workerCount = 1;

        auto out = pipe.CookAll(req);
        larvae::AssertEqual(out.m_total, size_t{2});
        larvae::AssertEqual(out.m_cooked, size_t{2});
        larvae::AssertEqual(out.m_failed, size_t{0});
        larvae::AssertEqual(out.m_skipped, size_t{0});
    });
    ...
    auto t7 = larvae::RegisterTest("NectarCookPipe", "CookAllParallel", []() {
        ...
        req.m_assets.PushBack(a);
        req.m_assets.PushBack(b);
        req.m_assets.PushBack(c);
        req.m_platform = "pc";
        req.m_workerCount = 2;

        auto out = pipe.CookAll(req);
        larvae::AssertEqual(out.m_total, size_t{3});
        larvae::AssertEqual(out.m_cooked, size_t{3});
        larvae::AssertEqual(out.m_failed, size_t{0});
    });
    ...
    auto t9 = larvae::RegisterTest("NectarCookPipe", "CookAllCacheSkip", []() {
        ...
        larvae::AssertEqual(out1.m_cooked, size_t{1});

        // Second cook — same data → cache hit
        auto out2 = pipe.CookAll(req);
        larvae::AssertEqual(out2.m_skipped, size_t{1});
    ...
    auto t15 = larvae::RegisterTest("NectarCookPipe", "CookAllEmpty", []() {
        ...
        auto out = pipe.CookAll(req);
        larvae::AssertEqual(out.m_total, size_t{0});
        larvae::AssertEqual(out.m_cooked, size_t{0});
    });
