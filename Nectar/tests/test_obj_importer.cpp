...
namespace
{
    ...
    constexpr const char* kCubeObj = "# Unit cube\n"
                                     "v -0.5 -0.5  0.5\n"
                                     "v  0.5 -0.5  0.5\n"
                                     "v  0.5  0.5  0.5\n"
                                     "v -0.5  0.5  0.5\n"
    ...
    constexpr const char* kTriangleObj = "v 0 0 0\n"
                                         "v 1 0 0\n"
                                         "v 0 1 0\n"
                                         "f 1 2 3\n";
    ...
    auto t1 = larvae::RegisterTest("NectarMesh", "ParseCubeOBJ", []() {
        ...
        larvae::AssertEqual(header->m_indexCount, uint32_t{36});
        // 6 faces * 4 unique vertex combos = 24 vertices (each face has unique normal)
        larvae::AssertEqual(header->m_vertexCount, uint32_t{24});
    ...
    auto t2 = larvae::RegisterTest("NectarMesh", "NmshBlobLayout", []() {
        ...
        larvae::AssertEqual(result.m_intermediateData.Size(), expected_size);
        ...
        larvae::AssertEqual(submeshes[0].m_indexCount, header->m_indexCount);
        ...
        for (uint32_t i = 0; i < header->m_vertexCount; ++i)
        {
            for (int a = 0; a < 3; ++a)
            {
                larvae::AssertTrue(verts[i].m_position[a] >= -0.5f - 1e-5f);
                larvae::AssertTrue(verts[i].m_position[a] <= 0.5f + 1e-5f);
            }
        }
    ...
    auto t3 = larvae::RegisterTest("NectarMesh", "AABB", []() {
        ...
        for (int a = 0; a < 3; ++a)
        {
            larvae::AssertTrue(std::fabs(header->m_aabbMin[a] - (-0.5f)) < kTol);
            larvae::AssertTrue(std::fabs(header->m_aabbMax[a] - 0.5f) < kTol);
        }
    });
    ...
    auto t4 = larvae::RegisterTest("NectarMesh", "ScaleSetting", []() {
        ...
        for (int a = 0; a < 3; ++a)
        {
            larvae::AssertTrue(std::fabs(header->m_aabbMin[a] - (-1.0f)) < kTol);
            larvae::AssertTrue(std::fabs(header->m_aabbMax[a] - 1.0f) < kTol);
        }
    });
    ...
    auto t6 = larvae::RegisterTest("NectarMesh", "FlipUV", []() {
        ...
        for (uint32_t i = 0; i < header->m_vertexCount; ++i)
        {
            if (verts[i].m_uv[1] > 0.9f)
                found_high = true;
            if (verts[i].m_uv[1] < 0.1f)
                found_low = true;
        }
    ...
    auto t11 = larvae::RegisterTest("NectarMesh", "QuadTriangulation", []() {
        ...
        larvae::AssertEqual(header->m_vertexCount, uint32_t{4});
    });
    ...
    auto t15 = larvae::RegisterTest("NectarMesh", "SubMeshAABB", []() {
        ...
        // Cube is [-0.5, 0.5]
        larvae::AssertTrue(submeshes[0].m_aabbMin[0] < 0.f);
        larvae::AssertTrue(submeshes[0].m_aabbMax[0] > 0.f);
    });
